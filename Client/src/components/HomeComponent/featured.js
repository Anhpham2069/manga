...
import {faStarOfLife,faFire,faBookmark,faEye,faCalendarDay} from "@fortawesome/free-solid-svg-icons"
...
console.log(storiesData)
  ...
  const [stories, ] = useState([
        {
          id: 1,
          imageUrl: 'https://doctruyen5s.top/uploads/covers/ngay-tan-the-ta-nhan-duoc-ty-le-rot-do-gap-100-lan.jpg',
          title: 'Truyện 1',
          genre: 'Kinh dị',
          saves: 50,
          views: 1000,
        ...
        {
          id: 2,
          imageUrl: 'https://doctruyen5s.top/uploads/covers/nhan-vat-phan-dien-dai-su-huynh-tat-ca-cac-su-muoi-deu-la-benh-kieu.jpg',
          title: 'Truyện 2',
          genre: 'Hài hước',
          saves: 30,
          views: 800,
      ...
      const renderList = () => {
        switch (selectedButton) {
          ...
          case 'tuannay':
            return Data.filter(story => new Date(story.date_added) >= new Date('2023-10-04'));
          case 'thangnay':
            return Data.filter(story => new Date(story.date_added).getMonth() === new Date().getMonth());
          default:
            return [];
        }
      }
    ...
    <div className='phone:flex-row lg:flex tablet:mx-6 lg:mx-14  mt-4 '>
        <div className={`${isDarkModeEnable? "bg-bg_dark_light text-text_darkMode": "bg-white"} lg:w-[70%] pl-3.5 shadow-lg `}>
            ...
            <div className='w-full my-10 flex justify-center items-center hover:text-primary-color'>
              <Link to='/all-stories'>
                <button className='w-24 mb-4 h-10 border-[1px] border-gray-300'>Xem Thêm</button>
              </Link>
            </div>
        </div>
        <div className='h-fit lg:ml-6 lg:w-[28%]'>
            <div className={` ${isDarkModeEnable? "bg-bg_dark_light text-text_darkMode": "bg-white"} shadow-lg h-fit `}>
                <div className='w-[40%] uppercase text-primary-color p-3 border-b-2 border-gray-200'><FontAwesomeIcon icon={faFire} className='mr-2' />Xem nhiều</div>
                <div className='p-3'>
                <div className={` ${isDarkModeEnable? "bg-[#333333] text-text_darkMode": "bg-[#F1F1F1]"} m-auto flex justify-center items-center h-10 rounded-md font-normal`}>
                    <FilterButton
                        text="Hôm nay"
                                ...
                                <div key={item.id} className='flex p-3 justify-start cursor-pointer border-b-2 border-gray-200'>
                                    ...
                                    <div className='ml-4 h-20 flex flex-col justify-around'>
                                        <div className='flex-1'>
                                            <p>{item.title}</p>
                                            <div className='flex justify-between text-xs'><p className='font-semibold text-[#888888] mr-1 '>Thể loại:</p> <p className=''> {item.genres}</p></div>
                                        </div>
            ...
            {/* new strories */}
            <div className={`${isDarkModeEnable? "bg-bg_dark_light text-text_darkMode": "bg-white"} h-fit shadow-lg mt-10`}>
                <div className='w-[50%] uppercase text-primary-color p-3 border-b-2 border-gray-200'><FontAwesomeIcon icon={faCalendarDay} className='mr-2' />Truyện mới</div>
                        {stories.map(item=>{   
                                ...
                                <div className='flex p-3 justify-start  border-b-2 border-gray-200'>
                                    ...
                                    <div className='ml-4 w-full h-20 flex flex-col justify-around'>
                                        <div className='flex-1'>
                                            ...
                                            <div className='flex text-base italic'><p>Chapter 2 - con rối giấy</p></div>
                                        </div>
...
const FilterButton = ({ text, isSelected, onClick,isDarkModeEnable }) => (
    <button
      onClick={onClick}
      className={`h-8 w-24 rounded-sm ${isDarkModeEnable? "  border-none": ""} ${
        isSelected ? 'bg-primary-color text-text_darkMode' : 'border-2 border-gray-300'
      }`}
    >
      {text}
    </button>
