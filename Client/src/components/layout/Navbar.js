                      ...
                      return(
                        <Link to={`/detail/${rs.slug}`}>
                          <div className="flex h-20 w-full p-2">
                            <img className="h-full" src={`https://img.otruyenapi.com//uploads/${searchResults.seoOnPage.og_image?.[index]}`} alt="anh"/>
                            <p className="text-black font-bold px-4 py-2 cursor-pointer hover:bg-[#F1F1F2]" key={rs._id}>{rs.name}</p>
                          </div>
                        </Link>
                      )
                  })}
